...
public class ApplyFunctionToColumn implements TableFramePlugin
{
    /**
     * Default empty constructor.
     */
    public ApplyFunctionToColumn()
    {
    }
    ...
    static class CustomDialog
    {
        ...
        public CustomDialog(ImagoFrame refFrame, String dlgTitle, Table refTable)
        {
            this.dlg = new JDialog(refFrame.getWidget(), dlgTitle, false);
            this.refFrame = refFrame;
            this.table = refTable;
            
            this.numColNames = NumericTable.keepNumericColumns(refTable).columns().stream()
                    .map(col -> col.getName())
                    .toArray(String[]::new);
            ...
            dlg.setLocationRelativeTo(refFrame.getWidget());
            dlg.pack();
        ...
        private void createWidgets()
        {
            ...
            String[] labels = ColumnFunction.getAllLabels();
        ...
        private void onOkButton()
        {
            ...
            ColumnFunction transformer = ColumnFunction.fromLabel((String) transformComboBox.getSelectedItem());
            ...
            if (resultIndex == 0)
            {
                ...
                for (int i = 0; i < inds.length; i++)
                {
                    ...
                    NumericColumn col = (NumericColumn) table.column(colIndex);
    ...
    enum ColumnFunction
    {
        ...
        private ColumnFunction(String name)
        {
            this.name = name;
        }
        ...
        public NumericColumn process(NumericColumn column)
        {
            ...
            return switch(this)
            {
                case LOG -> transform(column, name, Math::log);
                case SQRT -> transform(column, name, Math::sqrt);
                case LOG10 -> transform(column, name, Math::log10);
                case ZSCORE ->  
                {
                    double mean = SummaryStatistics.mean(column);
                    double std = SummaryStatistics.std(column);
                    yield transform(column, name, v -> (v - mean) / std);
                }
    
                default -> throw new RuntimeException("Unable to process Function: " + this.name);
            };
        }
        ...
        {
            return Stream.of(ColumnFunction.values())
                    .map(op -> op.name)
                    .toArray(String[]::new);
        }
         ...
         */
        public static ColumnFunction fromLabel(String label)
        {
            for (ColumnFunction item : ColumnFunction.values()) 
            {
